...
class Vehicle:
    ...
    def update(self, bbox_list):
        ...
        for bbox in bbox_list:
            ...
            if pdist(np.vstack((self.center, bbox_center)))[0] < 0.5 * max(self.size):
        ...
        if len(bbox_nearby) == 0:
            ...
            # add to this car's life because new bboxes confirm it as alive
            self.blood += 1
            self.blood = min(self.blood, 30)
            ...
            new_size[0] = min(200, new_size[0])
            new_size[1] = min(200, new_size[1])
            ...
            if np.abs(new_size[0] - self.size[0]) < 0.5 * self.size[0] or np.abs(new_size[1] - self.size[1]) < 0.5 * self.size[1]:
                ...
                self.size = self.size
            # print('>{}: new size: {}'.format(self.id, self.size))
        # limit the size of each vehicle for realistic reasons
        self.size[0] = min(200, self.size[0])
        self.size[1] = min(200, self.size[1])
